...
- 2025년 이후 공식 문서를 수집하며 관리자가 즉시 수집할 수 있고 정기 수집은 매일 자정 실행
...
- AI 검색은 벡터·키워드 후보에서 관련 카테고리를 고른 뒤 해당 카테고리 전체 문서를 재정렬
...
프론트엔드는 OpenAI나 데이터베이스에 직접 접근하지 않습니다. 백엔드가 인증과 비즈니스 로직을 처리하고, AI 서비스는 동기화된 위키만 검색해 답변과 실제 출처를 반환합니다. AI 검색은 벡터 후보 60개와 키워드 후보 70개에서 관련 카테고리를 최대 5개 선택하고, 해당 카테고리 전체 문서를 재정렬합니다. 최종 컨텍스트는 문서마다 최대 3청크를 라운드로빈으로 배치합니다.
