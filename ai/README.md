...
졸업요건, 등록금, 장학금 종합 문서는 숫자 위키 ID 대신 다음 `sourceKey`로 관리합니다.

- `software-graduation-requirements`
- `tuition-policy`
- `scholarship-policy`

관련 질문은 일반 벡터·키워드 후보보다 해당 키의 문서를 먼저 조회합니다. 벡터 저장은 Chroma에 다시 읽기를 수행해 청크 수와 키를 검증하며, 실패하면 오류로 처리되어 자동 동기화 큐 또는 관리자 재색인에서 재시도됩니다. AI 이미지 빌드는 전체 회귀 테스트를 실행하므로 핵심 질문의 우선 출처 테스트가 실패하면 배포되지 않습니다. RAG 검색어 캐시는 서비스 생성 시 초기화됩니다.
