...
질문마다 벡터·키워드 검색으로 관련 카테고리를 동적으로 최대 5개 선택한 뒤, 선택된 카테고리의 모든 문서를 가져와 다시 정렬합니다. 초기 키워드 후보는 최대 70건입니다. 특정 위키 ID, 문서 제목, 졸업·등록금·장학금 전용 키는 사용하지 않습니다. 사용자가 카테고리를 지정한 경우에는 해당 카테고리만 전체 검색합니다.
...
최종 답변 컨텍스트는 상위 문서마다 최대 3청크를 라운드로빈으로 배치합니다. 한 공지의 긴 본문이 컨텍스트를 독점하지 않아 종합 안내와 기준 문서도 함께 전달됩니다.
질문에 문서 제목이 정확히 포함되면 해당 문서 한 건은 최종 후보에서 보존합니다. 특정 ID나 주제 키 없이 모든 제목에 같은 규칙을 적용합니다.
