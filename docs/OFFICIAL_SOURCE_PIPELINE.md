...
기본 정책은 `하위 원문 1건 = 위키 1건`이다. 공지 제목·본문·첨부파일·원문 링크를 다른 공지와 섞지 않고 독립적으로 저장한다.

다음 세 출처만 관련 자료를 하나의 주제 위키로 통합한다.

- SW중심대학사업단 공지
- TOSC 공지: 시험 회차별 통합
- uDream 비교과·진로 프로그램

기존에 학기별로 합쳐졌던 수강편람과 강의시간표도 각각 독립 위키로 분리한다. 배포 후 topic rebuild가 이전 연결을 새 정책으로 재구성하고, 변경된 위키를 벡터 저장소에 다시 동기화한다.

벡터 동기화 큐는 같은 위키의 대기 작업을 새 행으로 계속 쌓지 않고 최신 payload로 갱신한다. 완료된 작업은 원문 payload를 제거하고 위키별 최신 이력만 남겨 운영 DB 용량 증가를 제한한다.
...
OFFICIAL_SOURCE_MAX_LIST_PAGES=300
