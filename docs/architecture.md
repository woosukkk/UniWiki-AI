...
질문 처리 중에는 ChromaDB 전체 본문을 조회하지 않습니다. 제한된 벡터 후보와 제목 메타데이터 일치 후보만 재정렬하고 선택된 위키 청크만 추가 조회하므로, 고유명사 검색을 유지하면서 본문 전체 적재로 인한 메모리 급증을 막습니다.
